    ...
    <ul class="list-group">
        <li class="px-0 mb-2 border-0 list-group-item d-flex align-items-center">
          ...
          <div class="d-flex align-items-start flex-column justify-content-center">
            ...
            <p v-if="!hasFile.docPath" class="mb-0 text-danger" style="font-size: 12px">{{ docType }} Not Uploaded</p>
            <p v-if="file_has_apostrophe" class="mb-0 text-danger text-xs">File cannot contain apostrophes.</p>
          </div>
     
          <button v-if="Object.keys(hasFile).length !== 0" @click="deleteDocument(hasFile.id)" type="button" class="mb-0 btn btn-link pe-3 ps-0 ms-auto">
          ...
          <i v-if="$page.props.auth.has_slowtow_admin_role" class="fa fa-upload" aria-hidden="true"></i>
